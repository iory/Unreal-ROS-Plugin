...
ACameraCapturer::ACameraCapturer() {
    // TextureRenderTarget =
    // CreateDefaultSubobject<UTextureRenderTarget2D>(TEXT("Camera"));
    // TextureRenderTarget->InitAutoFormat(256, 256);
    // this->GetCaptureComponent2D()->CaptureSource =
...
ACameraCapturer::~ACameraCapturer() {}
void ACameraCapturer::ShowCameraView(std::string CameraID,
                                     UTextureRenderTarget2D* CameraTexture2D) {
    TArray<FColor> FormatedImageData;
    FRenderTarget* RenderTarget =
      CameraTexture2D->GameThread_GetRenderTargetResource();
    if (RenderTarget) {
        if (RenderTarget->ReadPixels(FormatedImageData)) {
            if (FormatedImageData.Num() != 0) {
                // cv::Mat smallImage(cv::Size(256, 256), CV_8UC4, (unsigned
                // char *)FormatedImageData.GetData());
                // cv::imshow(CameraID.c_str(), smallImage);
            }
        }
    }
}
void ACameraCapturer::BeginPlay() { Super::BeginPlay(); }
