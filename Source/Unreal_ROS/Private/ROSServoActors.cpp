...
UROSMotorConstrainComponent::UROSMotorConstrainComponent(
  const FObjectInitializer& ObjectInitializer)
    : Super(ObjectInitializer) {
    bWantsBeginPlay = true;
    PrimaryComponentTick.bCanEverTick = true;
    ConstrainComponent = CreateDefaultSubobject<UPhysicsConstraintComponent>(
      TEXT("PhysicsConstraint"));
}

void UROSMotorConstrainComponent::BeginPlay() {
    UObjectPropertyBase* ObjProp = FindField<UObjectPropertyBase>(
      GetOwner()->GetClass(), ConstrainComponent->ComponentName2.ComponentName);
    if (ObjProp != nullptr) {
        rotor = Cast<UPrimitiveComponent>(
          ObjProp->GetObjectPropertyValue_InContainer(GetOwner()));
    } else {
        UE_LOG(LogTemp, Log, TEXT("NO rotor!!"));
    }
}

void UROSMotorConstrainComponent::TickComponent(
...
void UROSPoseDriver::TickComponent(
  float DeltaTime, enum ELevelTick TickType,
  FActorComponentTickFunction* ThisTickFunction) {
    if (root != nullptr) {
        if (PoseSub == nullptr) {
            PoseSub = U_geometry_msgs_PoseSubscriber::Create_Pose_Subscriber(
              SubscribeTopic);
        ...
        if (UsingNED) loc.Z = -pose_ros.position.z;
        FQuat rot(pose_ros.orientation.x, pose_ros.orientation.y,
